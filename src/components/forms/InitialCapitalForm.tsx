...
import { DollarSign, Rocket, ChevronRight } from "lucide-react";
import { useState } from "react";
import { formatNumber } from "@/lib/utils";
import { useInvestments } from "@/context/InvestmentContext";

export default function InitialCapitalForm({ onSuccess }: { onSuccess: () => void }) {
  const { setCapitalInicial } = useInvestments();
    ...
    <div className="space-y-8 py-4">
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-500 font-medium leading-relaxed max-w-xs mx-auto">
          Para comenzar, define el capital total que tienes disponible para gestionar e invertir.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 ml-1">Capital Inicial Total</label>
          <div className="relative group">
            <DollarSign className="absolute left-5 top-1/2 -translate-y-1/2 w-7 h-7 text-emerald-500 transition-colors" />
            <input 
              type="text" 
              value={amount}
        ...
        >
          <span>Inicializar Tablero</span>
          <ChevronRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
        </button>
      </form>

      <p className="text-[10px] text-center text-slate-400 font-medium px-8 leading-relaxed">
        Podrás ajustar este valor o añadir nuevas entradas de capital más tarde desde el módulo de Gestión de Capital.
      </p>
    </div>
  );
}
